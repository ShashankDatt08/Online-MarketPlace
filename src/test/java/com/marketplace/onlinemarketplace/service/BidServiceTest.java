...
public class BidServiceTest {
    ...
    @Test
    public void testCreateBidsProjectInProgress() {
        Projects project = new Projects();
        project.setStatus(Projects.Status.INPROGRESS);
        when(projectRepo.findById(anyLong())).thenReturn(Optional.of(project));
        assertThrows(RuntimeException.class, () -> bidService.createBids(1L, 1L, 100L, "proposal"));
    }
    ...
    @Test
    public void testCreateBidsSuccess() {
        Projects project = new Projects();
        project.setStatus(Projects.Status.OPEN);
        when(projectRepo.findById(anyLong())).thenReturn(Optional.of(project));
    ...
    public void testGetBidById() {
        ...
        bidService.getBidById(1L, false);
        assertNull(bid.getFreelancerId());
        assertEquals("Anonymous User", bid.getFreelancerName());
    }

    @Test
    public void testAcceptBidBidNotFound() {
    ...
    @Test
    public void testAcceptBidNotClientProject() {
        Bid bid = new Bid();
        bid.setProjectId(1L);
        when(bidRepo.findById(anyLong())).thenReturn(Optional.of(bid));
    ...
    public void testAcceptBidSuccess() {
        ...
        Bid result = bidService.acceptBid(1L, 1L);
        assertEquals(Bid.BidStatus.ACCEPTED, result.getStatus());
    }
}